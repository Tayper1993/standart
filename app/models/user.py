...
class User(db.Model):
    ...
    balance: so.Mapped[float] = so.mapped_column(sa.Float, default=0.0, comment='Баланс')
    commission_rate: so.Mapped[float] = so.mapped_column(sa.Float, default=0.0, comment='Ставка комиссии')
    webhook_url: so.Mapped[str] = so.mapped_column(sa.String(256), comment='URL webhook')
